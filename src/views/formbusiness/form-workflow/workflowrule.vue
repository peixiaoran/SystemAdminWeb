    ...
    <el-card class="conventional-card">
      ...
      <div class="table-container">
        ...
        >
          ...
          <el-table-column :label="$t('formbusiness.workflowrule.effectiveStartDate')" width="180" align="center">
            <template #default="{ row }">{{ formatDate(row.effectiveStartDate) }}</template>
          </el-table-column>
          <el-table-column :label="$t('formbusiness.workflowrule.effectiveEndDate')" width="180" align="center">
            <template #default="{ row }">{{ row.effectiveEndDate ? formatDate(row.effectiveEndDate) : $t('formbusiness.workflowrule.unlimited') }}</template>
          </el-table-column>
...
const handleEdit = async (row) => {
  ...
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      dialogForm.effectiveStartDate = formatDate(d.effectiveStartDate)
      dialogForm.effectiveEndDate   = formatDate(d.effectiveEndDate)
...
const handleSubmit = async () => {
  ...
  try {
    const params = {
      ...
      effectiveStartDate: dialogForm.effectiveStartDate || null,
      effectiveEndDate:   dialogForm.effectiveEndDate   || null
    }
