  ...
  <div class="leave-form-page">
    ...
    >
      <div v-if="resultState.variant === 'forbidden'" class="forbidden-result">
        ...
        <p class="forbidden-result__eyebrow">{{ t('formbusiness.informationrequest.forbiddenViewEyebrow') }}</p>
        <h2 class="forbidden-result__title">{{ t('formbusiness.informationrequest.forbiddenViewTitle') }}</h2>
        <p class="forbidden-result__desc">{{ t('formbusiness.informationrequest.forbiddenViewSubTitle') }}</p>
        ...
        <el-row v-if="isAnyStepFieldVisible(['Category', 'EstimatedDays'])" :gutter="16">
          <el-col v-if="isStepFieldVisible('Category')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.category')" prop="category">
              <KeywordHighlightField :value="categoryDisplay" :keyword="searchKeyword" />
            </el-form-item>
          </el-col>
          <el-col v-if="isStepFieldVisible('EstimatedDays')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.estimatedDays')" prop="estimatedDays">
              <KeywordHighlightField :value="form.estimatedDays" :keyword="searchKeyword" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row v-if="isStepFieldVisible('CurrentSituation')" :gutter="16">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.currentSituation')" prop="currentSituation">
              <KeywordHighlightField :value="form.currentSituation" :keyword="searchKeyword" multiline :rows="4" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row v-if="isStepFieldVisible('Expectations')" :gutter="16">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.expectations')" prop="expectations">
              <KeywordHighlightField :value="form.expectations" :keyword="searchKeyword" multiline :rows="4" />
            </el-form-item>
          </el-col>
        </el-row>
...
// 权限键缺失时 isStepFieldVisible 默认返回 true，故用 every：任一拼写判否即生效
...
async function loadCategoryOptions () {
  try {
    ...
    categoryOptions.value = res && res.code === 200 && Array.isArray(res.data) ? res.data : []
  } catch {
    categoryOptions.value = []
  }
}

/** 需求类别无独立 Name 字段返回，依赖下拉数据把 code 映射成名称展示 */
const categoryDisplay = computed(() => {
  const matched = categoryOptions.value.find((item) => String(item.category) === String(form.category))
  return matched?.categoryName || form.category
})
  ...
  try {
    ...
    if (!res || res.code !== 200) {
      ...
      if (isBadRequestResponse(res)) {
        ...
        showResultNotice(message, Number(res?.code) === 400 ? 'warning' : 'error')
      }
      return
    }
    const data = res.data || {}
    ...
    showResultNotice(t('formbusiness.informationrequest.workflowLoadFailed'), 'error')
  } finally {
...
/** 业务 API code 状态码提示：统一走右上角通知，与编辑页保持一致 */
function showResultNotice (message, type = 'error') {
  const text = typeof message === 'string' ? message.trim() : ''
  ElNotification({
    title: '',
    message: text || t('common.operationFailed'),
    type,
    position: 'top-right'
  })
}
...
function showForbiddenResult () {
  ...
  resultState.titleKey = 'formbusiness.informationrequest.forbiddenViewTitle'
  resultState.subTitleKey = 'formbusiness.informationrequest.forbiddenViewSubTitle'
}
  ...
  try {
    ...
    if (res.code !== 200) {
      if (isBadRequestResponse(res)) {
        ...
        showResultNotice(res.message, Number(res?.code) === 400 ? 'warning' : 'error')
      }
      return
    }
