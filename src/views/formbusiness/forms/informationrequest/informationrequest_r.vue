  ...
  >
        ...
        <el-skeleton animated>
          <template #template>
            ...
            <div class="sk-body">
              ...
              <div class="sk-grid">
                <div v-for="n in 2" :key="`sk-category-${n}`" class="sk-field">
                  <el-skeleton-item variant="text" class="sk-label" />
                  <el-skeleton-item variant="text" class="sk-control" />
                </div>
              </div>
    ...
    >
      <div v-if="resultState.variant === 'forbidden'" class="forbidden-result">
        ...
        <p class="forbidden-result__eyebrow">{{ t('formbusiness.informationrequest.forbiddenReviewEyebrow') }}</p>
        <h2 class="forbidden-result__title">{{ t('formbusiness.informationrequest.forbiddenReviewTitle') }}</h2>
        <p class="forbidden-result__desc">{{ t('formbusiness.informationrequest.forbiddenReviewSubTitle') }}</p>
        <span class="result-back-link" @click="closeCurrentPage">[ {{ t('formbusiness.informationrequest.backToFormPending') }} ]</span>
      </div>
      <el-result
        v-else
      ...
      >
        <template #sub-title>
          <p v-if="resultState.variant === 'badRequest'" class="informationrequest-bad-request-desc">
            {{ t('formbusiness.informationrequest.badRequestHint') }}
          </p>
    ...
    <el-card class="leave-form-card" shadow="never">
      <div class="form-title-row">
        <h2 class="form-title">{{ t('formbusiness.informationrequest.formTitle') }}</h2>
      </div>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px" class="leave-form" :validate-on-rule-change="false">
        ...
        <el-row v-if="isAnyStepFieldVisible(['FormNo', 'ApplyDate'])" :gutter="16" class="basic-info-row" style="justify-content: flex-start;">
          <el-col v-if="isStepFieldVisible('FormNo')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.formNo')" prop="formNo">
              <el-input v-model="form.formNo" :disabled="!isStepFieldEditable('FormNo')" />
            </el-form-item>
          </el-col>
          <el-col v-if="isStepFieldVisible('ApplyDate')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.applyDate')" prop="applyDate">
              <el-date-picker
                v-model="form.applyDate"
                type="date"
                value-format="YYYY-MM-DD"
                :placeholder="t('formbusiness.informationrequest.pleaseSelectApplyDate')"
        ...
        >
          <el-col v-if="isStepFieldVisible('UserNo')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.applicantUserNo')" prop="applicantUserNo">
              <el-input v-model="form.applicantUserNo" :disabled="!isStepFieldEditable('UserNo')" />
            </el-form-item>
          </el-col>
          <el-col v-if="isStepFieldVisible('UserName')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.applicantUserName')" prop="applicantUserName">
          ...
          <el-col v-if="isStepFieldVisible('Department')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.applicantDeptName')" prop="applicantDeptName">
              <el-input v-model="form.applicantDeptName" :disabled="!isStepFieldEditable('Department')" style="width: 100%;" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row v-if="isAnyStepFieldVisible(['Category', 'EstimatedDays'])" :gutter="16">
          <el-col v-if="isStepFieldVisible('Category')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.category')" prop="category">
              <el-select
                v-model="form.category"
                :placeholder="t('formbusiness.informationrequest.pleaseSelectCategory')"
                clearable
                :disabled="!isStepFieldEditable('Category')"
                style="width: 100%;"
              >
                <el-option
          ...
          <el-col v-if="isStepFieldVisible('EstimatedDays')" :span="8">
            <el-form-item :label="t('formbusiness.informationrequest.estimatedDays')" prop="estimatedDays">
              <el-input-number v-model="form.estimatedDays" :min="0" :precision="2" :step="0.01" controls-position="right" :disabled="!isStepFieldEditable('EstimatedDays')" style="width: 100%;" />
            </el-form-item>
          </el-col>
        </el-row>

        <el-row v-if="isStepFieldVisible('CurrentSituation')" :gutter="16">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.currentSituation')" prop="currentSituation">
              <el-input
                v-model="form.currentSituation"
                type="textarea"
                :rows="4"
                :placeholder="t('formbusiness.informationrequest.pleaseInputCurrentSituation')"
                :disabled="!isStepFieldEditable('CurrentSituation')"
              />
            </el-form-item>
          </el-col>
        </el-row>

        <el-row v-if="isStepFieldVisible('Expectations')" :gutter="16">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.expectations')" prop="expectations">
              <el-input
                v-model="form.expectations"
                type="textarea"
                :rows="4"
                :placeholder="t('formbusiness.informationrequest.pleaseInputExpectations')"
                :disabled="!isStepFieldEditable('Expectations')"
              />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row v-if="isStepFieldVisible('Upload') || uploadedAttachments.length > 0" :gutter="16" class="attachment-row">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.attachments')">
              <div class="upload-section">
                ...
                <div v-if="isStepFieldVisible('Upload')" class="upload-actions">
                  <el-button class="upload-trigger" type="primary" plain :loading="uploading" :disabled="uploading || !isStepFieldEditable('Upload')" @click="openFilePicker">
                    ...
                    {{ t('formbusiness.informationrequest.uploadFile') }}
                  </el-button>
                </div>
                <el-table :data="uploadedAttachments" border size="small" class="attachment-table">
                  ...
                  <el-table-column :label="t('formbusiness.informationrequest.fileName')" min-width="150">
                  ...
                  <el-table-column :label="t('formbusiness.informationrequest.fileSize')" width="100" align="center">
                    <template #default="{ row }">
                  ...
                  <el-table-column :label="t('common.operation')" width="200" align="center">
                    <template #default="{ row, $index }">
                      ...
                      >
                        {{ t('formbusiness.informationrequest.download') }}
                      </el-button>
                      <el-button
                      ...
                      >
                        {{ t('formbusiness.informationrequest.deleteFile') }}
                      </el-button>
                    </template>
                  </el-table-column>
                </el-table>
        ...
        <el-row v-if="isAddReviewVisible()" :gutter="16" class="add-review-row">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.addReview')">
              <el-table ref="addReviewTableRef" :data="addReviewRows" border size="small" class="add-review-table" row-key="_uid">
                <el-table-column width="40" align="center">
                  <template #default>
                    <el-icon
                      ...
                      :title="t('formbusiness.informationrequest.addReviewDragTip')"
                    >
                      <Rank />
                    </el-icon>
                  </template>
                ...
                <el-table-column
                  prop="sortOrder"
                  :label="t('formbusiness.informationrequest.addReviewSortOrder')"
                  width="70"
                  align="center"
                />
                <el-table-column
                ...
                <el-table-column v-if="isAddReviewEditable()" :label="t('common.operation')" width="150" align="center">
                  <template #default="{ row }">
                    ...
                    >
                      {{ row.userId ? t('formbusiness.informationrequest.addReviewChange') : t('formbusiness.informationrequest.addReviewSelect') }}
                    </el-button>
                    <el-button
                      v-if="row.userId"
                      type="danger"
                    ...
                    >
                      {{ t('formbusiness.informationrequest.addReviewClear') }}
                    </el-button>
                  </template>
                </el-table-column>
              </el-table>
        ...
        <el-row v-if="isStepFieldVisible('Rating')" :gutter="16" class="rating-row">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.rating')" prop="rating">
              <el-rate v-model="form.rating" show-text :texts="ratingTexts" :disabled="!isStepFieldEditable('Rating')" @change="handleRatingChange" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row v-if="isStepFieldVisible('Comments')" :gutter="16" class="approval-comment-row">
          <el-col :span="24">
            <el-form-item :label="t('formbusiness.informationrequest.approvalComment')">
              <el-input
                v-model="approvalComment"
                type="textarea"
                :rows="3"
                :placeholder="t('formbusiness.informationrequest.approvalCommentPlaceholder')"
            ...
            <el-form-item class="form-actions-form-item">
              <div class="form-actions-row">
                <div class="form-actions-buttons">
                  <el-button v-if="isStepFieldVisible('Save')" type="primary" round style="width:80px;" @click="onSubmit" :loading="saving" :disabled="formActionLoading || !isStepFieldEditable('Save')">{{ t('formbusiness.informationrequest.saveButton') }}</el-button>
                  <el-button v-if="isStepFieldVisible('Submit')" type="success" round style="width:80px;" @click="onSubmitForApproval" :loading="approving" :disabled="formActionLoading || !isStepFieldEditable('Submit')">{{ t('formbusiness.informationrequest.submitButton') }}</el-button>
                  <el-button v-if="isStepFieldVisible('Reject')" type="danger" round style="width:80px;" @click="onReject" :disabled="formActionLoading || !isStepFieldEditable('Reject')">{{ t('formbusiness.informationrequest.rejectButton') }}</el-button>
                </div>
                <div class="workflow-view-entry">
                  <div class="workflow-view-hint">
                    <span class="workflow-view-hint-text">{{ t('formbusiness.informationrequest.viewFullWorkflowHint') }}</span>
                  </div>
                  <el-tooltip :content="t('formbusiness.informationrequest.viewFullWorkflow')" placement="top">
                    <span
    ...
    <ReviewLogCard :records="reviewRecordList" i18n-prefix="formbusiness.informationrequest" />
    ...
    <RejectDialog
      ...
      i18n-prefix="formbusiness.informationrequest"
      @confirm="handleRejectConfirm"
    />

    <el-dialog
      v-model="addReviewDialogVisible"
      :title="t('formbusiness.informationrequest.addReviewDialogTitle')"
      width="1100px"
    ...
    >
      <el-form :inline="true" class="add-review-filter-form">
        <el-form-item :label="t('formbusiness.informationrequest.addReviewDepartmentFilter')">
          <el-tree-select
            ...
            popper-class="informationrequest-dept-tree-popper"
            :placeholder="t('formbusiness.informationrequest.addReviewPleaseSelectDepartment')"
            @change="handleAddReviewDepartmentChange"
          />
        </el-form-item>
        <el-form-item :label="t('formbusiness.informationrequest.applicantUserNo')">
          <el-input
            v-model="addReviewFilters.userNo"
            class="add-review-filter-input-userno"
            clearable
            :placeholder="t('formbusiness.informationrequest.applicantUserNo')"
            @input="handleAddReviewFilterInput"
            @keyup.enter="handleAddReviewSearch"
            @clear="handleAddReviewFilterInput"
          />
        </el-form-item>
        <el-form-item :label="t('formbusiness.informationrequest.applicantUserName')">
          <el-input
            v-model="addReviewFilters.userName"
            class="add-review-filter-input-compact"
            clearable
            :placeholder="t('formbusiness.informationrequest.applicantUserName')"
            @input="handleAddReviewFilterInput"
            @keyup.enter="handleAddReviewSearch"
            @clear="handleAddReviewFilterInput"
          />
      ...
      >
        ...
        >
          ...
          <el-table-column prop="userNo" :label="t('formbusiness.informationrequest.applicantUserNo')" min-width="110" align="center" />
          <el-table-column prop="userName" :label="t('formbusiness.informationrequest.applicantUserName')" min-width="120" align="left" show-overflow-tooltip />
          <el-table-column :label="t('formbusiness.informationrequest.addReviewDepartment')" min-width="160" align="left" show-overflow-tooltip>
...
import {
  INIT_INFORMATIONREQUEST_API,
  GET_INFORMATIONREQUEST_API,
  SAVE_INFORMATIONREQUEST_API,
  UPLOAD_FILE_API,
  DELETE_FILE_API,
  GET_FULL_REVIEW_FLOW_API,
  GET_REJECT_STEP_DROP_API,
  APPROVE_INFORMATIONREQUEST_API,
  REJECT_INFORMATIONREQUEST_API,
  ...
  GET_IT_CATEGORY_DROP_API
} from '@/config/api/formbusiness/forms/informationrequest'
...
const resultState = reactive({
  ...
  detailMessage: '',
  status: 'success',
  titleKey: 'formbusiness.informationrequest.approvalResultTitle',
  subTitleKey: 'formbusiness.informationrequest.approvalResultSubTitle'
})
...
const rules = computed(() => {
  ...
  return {
    category: [{ required: true, message: t('formbusiness.informationrequest.pleaseSelectCategory'), trigger: 'change' }],
    currentSituation: requiredWhenEditable('CurrentSituation', 'formbusiness.informationrequest.pleaseInputCurrentSituation', 'blur'),
    expectations: requiredWhenEditable('Expectations', 'formbusiness.informationrequest.pleaseInputExpectations', 'blur'),
    estimatedDays: isStepFieldEditable('EstimatedDays')
      ? [
          { required: true, message: t('formbusiness.informationrequest.pleaseInputEstimatedDays'), trigger: 'change' },
          {
            validator: (_rule, value, callback) => {
              if (value === null || value === undefined || value === '' || Number(value) > 0) {
                callback()
              } else {
                callback(new Error(t('formbusiness.informationrequest.estimatedDaysMustBeGreaterThanZero')))
              }
            },
            trigger: 'change'
          }
      ...
      : [],
    rating: isStepFieldEditable('Rating')
      ? [
          {
            validator: (_rule, value, callback) => {
              if (Number(value) > 0) {
                ...
                callback(new Error(t('formbusiness.informationrequest.pleaseSelectRating')))
              }
            },
            trigger: 'change'
          }
...
const ratingTexts = computed(() => [
  t('formbusiness.informationrequest.ratingTextVeryPoor'),
  t('formbusiness.informationrequest.ratingTextPoor'),
  t('formbusiness.informationrequest.ratingTextAverage'),
  t('formbusiness.informationrequest.ratingTextGood'),
  t('formbusiness.informationrequest.ratingTextExcellent')
])

/** el-rate 点击后不会像 el-input/el-select 那样自动通知 el-form-item 重新校验，需手动清除必填红字 */
function handleRatingChange (value) {
  if (Number(value) > 0) {
    formRef.value?.clearValidate('rating')
  }
}
...
async function loadCategoryOptions () {
  try {
    const res = await post(GET_IT_CATEGORY_DROP_API, {})
    const raw = res && isSuccessCode(res.code) && Array.isArray(res.data) ? res.data : []
    categoryOptions.value = raw.map((item) => ({
      code: item.category,
...
onMounted(() => {
  if (!loading.value && !resultState.visible) setupAddReviewSortable()
})

watch(() => loading.value || resultState.visible, (blocked) => {
  if (!blocked) setupAddReviewSortable()
...
async function saveAddReviewRowsBatch (rows) {
  ...
  try {
    ...
    if (res && isSuccessCode(res.code)) {
      ...
      showFormActionNotice(res?.message || t('formbusiness.informationrequest.addReviewReorderFailed'), 'warning')
    }
  } catch {
    // 静默：失败时保留 dirty，保存表单时再补一次
  }
  ...
  try {
    ...
    if (!res || res.code !== 200) {
      ...
      if (isBadRequestResponse(res)) {
        ...
        const message = res?.message || t('formbusiness.informationrequest.workflowLoadFailed')
    ...
    showFormActionNotice(t('formbusiness.informationrequest.workflowLoadFailed'), 'error')
  } finally {
...
function openWorkflowDrawer () {
  if (!form.formId) {
    ElMessage.warning(t('formbusiness.informationrequest.workflowNeedFormId'))
    return
  }
...
function showForbiddenResult () {
  ...
  resultState.titleKey = 'formbusiness.informationrequest.forbiddenReviewTitle'
  resultState.subTitleKey = 'formbusiness.informationrequest.forbiddenReviewSubTitle'
}

function showBadRequestResult (message) {
  const msg = typeof message === 'string' ? message.trim() : ''
  resultState.variant = 'badRequest'
  resultState.detailMessage = msg || t('formbusiness.informationrequest.badRequestFallbackMessage')
  resultState.visible = true
  resultState.status = 'warning'
...
async function getInformationRequestDetail (formId) {
  try {
    const res = await post(
      GET_INFORMATIONREQUEST_API,
...
/** InitInformationRequest：新建资讯需求单初始化。返回完整实体则直接 bind，仅返回 formId 时再拉详情 */
async function initInformationRequest () {
  try {
    const formData = new window.FormData()
    formData.append('formTypeId', currentFormTypeId.value || String(route.query.formTypeId || ''))
    const res = await post(INIT_INFORMATIONREQUEST_API, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
      silentForbiddenError: false
    })
    if (isForbiddenCode(res?.code)) {
    ...
    await getInformationRequestDetail(newFormId)
  } catch {}
}

function buildSaveInformationRequestPayload () {
  return {
    formId: String(form.formId || ''),
...
async function saveInformationRequestRequest () {
  // 先同步加审人员，再保存表单；保存与送审两条路径都经过这里
  await syncAddReviewRows()
  return post(SAVE_INFORMATIONREQUEST_API, buildSaveInformationRequestPayload(), {
    silentForbiddenError: false
  })
}
...
async function onSubmit () {
  ...
  saving.value = true
  try {
    const res = await saveInformationRequestRequest()
    if (isForbiddenCode(res?.code)) {
      showFormActionNotice(t('formbusiness.informationrequest.forbiddenResultSubTitle'), 'warning')
      ...
      showFormActionNotice(res?.message || t('formbusiness.informationrequest.badRequestFallbackMessage'), 'warning')
    } else {
...
async function saveInformationRequestBeforeSubmit () {
  const saveRes = await saveInformationRequestRequest()
  ...
  if (!saveRes || !isSuccessCode(saveRes.code)) {
    if (isValidationWarningCode(saveRes?.code)) {
      ...
      showFormActionNotice(saveRes?.message || t('formbusiness.informationrequest.badRequestFallbackMessage'), 'warning')
    } else {
...
async function onSubmitForApproval () {
  ...
  try {
    await ElMessageBox.confirm(
      t('formbusiness.informationrequest.submitConfirmMessage'),
      t('formbusiness.informationrequest.submitConfirmTitle'),
  ...
  try {
    const saved = await saveInformationRequestBeforeSubmit()
    if (!saved) return

    const formId = String(form.formId || '')
    if (!formId) {
      showFormActionNotice(t('formbusiness.informationrequest.workflowNeedFormId'), 'warning')
      return
    }

    const res = await post(APPROVE_INFORMATIONREQUEST_API, {
      formId,
      rejectStepId: '0',
    ...
    if (res && isSuccessCode(res.code)) {
      showResult('success', 'formbusiness.informationrequest.approvalResultTitle', 'formbusiness.informationrequest.approvalResultSubTitle')
      return
    }
    ...
    showFormActionNotice(res?.message || t('formbusiness.informationrequest.submitFailed'), 'error')
  } catch {} finally {
...
async function handleRejectConfirm ({ rejectStepId, rejectReason }) {
  ...
  try {
    const res = await post(REJECT_INFORMATIONREQUEST_API, {
      formId,
      rejectStepId,
    ...
    if (res && isSuccessCode(res.code)) {
      showResult('success', 'formbusiness.informationrequest.rejectResultTitle', 'formbusiness.informationrequest.rejectResultSubTitle')
      return
    }
    if (isBadRequestResponse(res)) {
      showBadRequestResult(res?.message)
    ...
    ElNotification({ title: '', message: res?.message || t('formbusiness.informationrequest.rejectFailed'), type: 'error' })
  } catch {
    ElNotification({ title: '', message: t('formbusiness.informationrequest.rejectFailed'), type: 'error' })
  } finally {
...
async function batchUpload (filesToUpload) {
  ...
  try {
    ...
    if (res && isSuccessCode(res.code)) {
      ...
      showFormActionNotice(res?.message || t('formbusiness.informationrequest.uploadFailed'), 'error')
    }
  } catch {
    showFormActionNotice(t('formbusiness.informationrequest.uploadFailed'), 'error')
  } finally {
...
async function removeAttachment (file, idx) {
  ...
  try {
    ...
    if (res && isSuccessCode(res.code)) {
      ...
      showFormActionNotice(res?.message || t('formbusiness.informationrequest.deleteFailed'), 'error')
    }
  } catch {
    showFormActionNotice(t('formbusiness.informationrequest.deleteFailed'), 'error')
  } finally {
    attachmentActionKeys.delete(key)
  }
}
  ...
  try {
    ...
    if (routeToken) {
      ...
      if (tokenFormId) {
        ...
        await getInformationRequestDetail(tokenFormId)
      }
      return
    }
    ...
    if (routeFormId) {
      ...
      await getInformationRequestDetail(form.formId)
    } else {
      await initInformationRequest()
    }
  } catch {
...
.informationrequest-bad-request-desc {
  max-width: 520px;
  margin: 0 auto;
  text-align: center;
  font-size: 13px;
