<template>
  <div class="conventional-table-container">
    <el-card class="conventional-card">
      <el-form :inline="true" class="conventional-filter-form" role="search" :aria-label="$t('formbusiness.reviewhistory.filterAriaLabel')">
        <el-form-item>
          <el-button :icon="Filter" @click="filterDialogVisible = true">
            {{ $t('formbusiness.reviewhistory.filterQuery') }}
          </el-button>
          <el-button :icon="RefreshLeft" :title="$t('formbusiness.reviewhistory.clearAll')" @click="handleClearFiltersAndSearch" />
        </el-form-item>
      ...
      <el-dialog
        ...
        :title="$t('formbusiness.reviewhistory.filterQuery')"
        width="900px"
        destroy-on-close
        append-to-body
      >
        <el-form :model="searchForm" :inline="true" label-width="120px" class="dialog-form filter-dialog">
          ...
          <div class="form-row">
            <el-form-item :label="$t('formbusiness.reviewhistory.formNo')">
              <el-input v-model="searchForm.formNo" clearable style="width: 100%;" />
            </el-form-item>
            <el-form-item :label="$t('formbusiness.reviewhistory.keyword')">
        ...
        <template #footer>
          <div class="filter-dialog-footer">
            <el-button @click="handleClearFilters">
              {{ $t('formbusiness.reviewhistory.clearAll') }}
            </el-button>
...
const handlePrintForm = async (row) => {
  ...
  try {
    ...
    showMessage(await resolveBlobErrorMessage(error, 'formbusiness.reviewhistory.printFailed'))
  } finally {
...
const handleExportExcel = async () => {
  ...
  try {
    ...
    showMessage(await resolveBlobErrorMessage(error, 'formbusiness.reviewhistory.exportFailed'))
  } finally {
...
const handleBatchPrintForm = async () => {
  ...
  try {
    ...
    showMessage(await resolveBlobErrorMessage(error, 'formbusiness.reviewhistory.batchPrintFailed'))
  } finally {
